...
\newcommand\ExamDate{Tuesday, June 16, 2015}
...
A dictionary is implemented using radix tree and \verb|vector<string>|. Structure of radix tree node and string is given.
\begin{lstlisting}
class string
{
	char* data;
...
class RadixNode
{
	...
	int Check;
	int NoOfChildren;
	RadixNode* Children[256];
};
\end{lstlisting}
It is known beforehand that dictionary contains words of average length 8. One fourth the total number of characters in radix tree occur as common prefix. For example, a radix tree contains words `con' and `court': Total characters are 8 and 2 characters are common. Find the space occupied by radix tree and vector for a dictionary containing 1 million words. Also find space complexity functions for both data structures as a function of dictionary size. Use byte as the unit of complexity function.
\newpage
\noindent\textbf{Question 2: Concepts\hfill \Qtwo~marks}
\begin{itemize}
\item[a.] What is threading? Explain in the context of parallelism.\newpage
\item[b.] Data structures with dynamic allocation cannot be sent/received in a single statement because size is not fixed. Given a dictionary defined as \verb|vector<string> Dictionary|, how would you send this over network? Write code for both sender and receiver assuming a connection is already established.
\end{itemize}
%\begin{table}[H]
%\begin{center}
%\vspace{0.3cm}
...
%\end{tikzpicture}
\newpage
\noindent\textbf{Question 6: Graph\hfill \Qsix~marks}
\begin{center}
\begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=3cm,thick,main node/.style={circle,fill=blue!10,draw,font=\sffamily\Large\bfseries}]
...
\path[every node/.style={font=\sffamily\small}]
	...
	(d) edge node [above] {7} (h)
	(e) edge node [left] {5} (f)
	(e) edge node [right] {11} (h)
	(f) edge node [above] {2} (h)
	(f) edge node [left] {1} (g)
	(h) edge node [right] {2} (g)
	(g) edge node [above] {1} (c);
\end{tikzpicture}\\[0.2cm]
\begin{itemize}
\item[a.] Apply Dijkstra's algorithm and find shortest path from `b' to all other nodes. Just label all the nodes.\vspace{7cm}
\item[b.] Assuming the graph is undirected, find MST such that distance between `f' and `h' is minimum.
\end{itemize}
\end{center}
\newpage
\noindent\textbf{Question 7: Project -- I\hfill \Qseven~marks}\\
\noindent\textbf{Question 8: Project -- I Bonus\hfill \Qeight~marks}\\
You can use this page for rough work.
\end{document}