...
SIZE = 2000; % No. of spatial steps
SlabLeft = 1800; % Location of left end of Slab.
SlabRight = SlabLeft+50; % Location of right end of Slab
...
aimp = 20; % Impulse scaling parameter
Amplitude = 2; % Square pulse and unit step amplitude
source = 20; % Location of source
SaveFields = 1; % 0. No, 1. Yes.
SnapshotInterval = 32; % Amount of time delay between snaps.
PECLoad = -1; % Location of PEC load
PMCLoad = -1; % Location of PMC Load
% Choice of source.
% 1. Gaussian 2. Sine wave 3. Ricker wavelet 4. Square Pulse 5. Unit Step 6.
% Impulse
SourceChoice = 2;
...
einf = ones(SIZE,1);
einf(SlabLeft:SlabRight) = 4; % einf(Drude) or er in slab.
uinf = ones(SIZE,1);
uinf(SlabLeft:SlabRight) = 1; % uinf(Drude) or ur in slab.
wpesq = zeros(SIZE,1);
wpesq(SlabLeft:SlabRight) = 0*2*w^2; % DNG(Drude) value of wpe squared in slab.
wpmsq = zeros(SIZE,1);
wpmsq(SlabLeft:SlabRight) = 0*2*w^2; % DNG(Drude) value of wpm squared in slab.
ge = zeros(SIZE,1);
ge(SlabLeft:SlabRight) = 0*w/32; % Electric collision frequency in slab.
gm = zeros(SIZE,1);
gm(SlabLeft:SlabRight) = 0*w/32; % Magnetic collision frequency in slab.
...
if SaveFields == 1
    ...
    HySnapshots = zeros(SIZE, MaxTime/SnapshotInterval); % Data for plotting.
    frame = 1;
end
...
%     elseif SourceChoice == 4
%     if q > td && q <= td+PulseWidth
%         Ex(source,n2) = Ex(source,n2) + Amplitude * Sc;
%     end
%     elseif SourceChoice == 5
%     Ex(source,n2) = Ex(source,n2) + Amplitude * Sc;
%     elseif SourceChoice == 6
%     Ex(source,n2) = Ex(source,n2) + 1/(aimp*sqrt(pi))*exp(-1*((q-td)/aimp)^2) * Sc;
%     end
%     
...
for q = 0:MaxTime
    ...
    % Source.
    if SourceChoice == 1
    Ex(source,n2) = Ex(source,n2) + Amplitude * exp( -1*((q-td)/(PulseWidth/4))^2 ) * Sc;
    elseif SourceChoice == 2
    Ex(source,n2) = Ex(source,n2) + Amplitude * sin(2*pi*f*(q)*dt) * Sc;
    elseif SourceChoice == 3
    Ex(source,n2) = Ex(source,n2) + Amplitude * (1-2*(pi*fp*(q*dt-dr))^2)*exp(-1*(pi*fp*(q*dt-dr))^2) * Sc;
    elseif SourceChoice == 4
    if q > td && q <= td+PulseWidth
        Ex(source,n2) = Ex(source,n2) + Amplitude * Sc;
    end
    ...
    Exi(q+1) = Ex(source,n2); % Incident field is left of slab.
    Ext(q+1) = Ex(x1,n2);
...
hold off
plot([dz*SlabLeft dz*SlabLeft], [-2 2], 'Color', 'r');
hold on
plot([dz*SlabRight dz*SlabRight], [-2 2], 'Color', 'r');
linelength=1e-3*(SlabLeft-source);
linex = -1*linelength:0.001:0;
Z0 = 377;
ZL = 377/sqrt(einf(SlabLeft))/sqrt(uinf(SlabLeft));
Beta = l/(2*pi);
Zin = Z0.*(ZL+i*Z0*tan(Beta.*linex))./(Z0+i*ZL*tan(Beta.*linex));
plot(linex,Zin, 'LineWidth', 2.0, 'Color', 'b');
set(gca, 'FontSize', 10, 'FontWeight', 'b')
axis([0 dz*SIZE -377 377])
title('Time Domain Simulation', 'FontSize', 12, 'FontWeight', 'b')
xlabel('z-axis (meters)', 'FontSize', 11, 'FontWeight', 'b')
ylabel('Input Impedance', 'FontSize', 11, 'FontWeight', 'b')
grid on
...
if SaveFields == 1
    ...
    for i=1:frame-1
        ...
        hold on
        plot([dz*SlabRight dz*SlabRight], [-2 2], 'Color', 'r');
        plot(dz*(1:length(HySnapshots(:,i))),HySnapshots(:,i), 'LineWidth', 2.0, 'Color', 'b');
        set(gca, 'FontSize', 10, 'FontWeight', 'b')
        axis([0 dz*SIZE -2/377 2/377])
        title('Time Domain Simulation', 'FontSize', 12, 'FontWeight', 'b')
        xlabel('z-axis (meters)', 'FontSize', 11, 'FontWeight', 'b')
        ylabel('Magnetic field (Hy)', 'FontSize', 11, 'FontWeight', 'b')
        grid on
%         figure (9)
%         Scatterer boundaries.
%         hold off
...
%         hold on
%         plot([dz*SlabRight dz*SlabRight], [-2 2], 'Color', 'r');
%         plot(dz*(1:length(HySnapshots(:,i))),ExSnapshots(:,i)./HySnapshots(:,i), 'LineWidth', 2.0, 'Color', 'b');
%         set(gca, 'FontSize', 10, 'FontWeight', 'b')
%         axis([0 dz*SIZE -377 377])
%         title('Time Domain Simulation', 'FontSize', 12, 'FontWeight', 'b')
%         xlabel('z-axis (meters)', 'FontSize', 11, 'FontWeight', 'b')
