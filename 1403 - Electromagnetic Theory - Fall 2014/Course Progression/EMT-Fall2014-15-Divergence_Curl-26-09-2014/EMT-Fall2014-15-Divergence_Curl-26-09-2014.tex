...
\section{Exercises}
\noindent\textbf{Question 1 \cite{Sadiku}[PE 3.7]:} Determine the flux of \textbf{A}$=\rho^2\cos^2\phi\hat\rho+z\sin\phi\hat\phi$ over the closed surface of a cylinder of height 1 m and radius 4 m. Verify Divergence theorem for this case.\\[0.2cm]
\noindent \textbf{Question 2:} Given a vector field $\textbf{A}=y^2\hat x+2xy\hat y$ find the circulation of \textbf{A} ($\oint\limits_{L} \textbf{A}\cdot d\textbf{\textit{l}}$) over closed path ABCDA. Verify Stokes's theorem for this case.
\begin{figure}[H]
\centering
